...
<script>
    $(window).on("load", function() {
        ...
        var smsalertinfo = localStorage.getItem("balancealertinfo");
        // console.log(dateinfo);
        if ((balance < 100) && (smsalertinfo != dateinfo)) {
            localStorage.clear();

            Swal.fire({
  icon: "warning",
  title: 'Payment Now',
  text: "Your Balance Low",
  showDenyButton: true,
  ...
  if (result.isDenied==true) {
    Swal.fire(" We Will Remember Tomorrow", {
           icon: "success",
             });
  localStorage.setItem("balancealertinfo", dateinfo);
  } else {
     window.location.href = url + '/mso/payments';
  }
})
